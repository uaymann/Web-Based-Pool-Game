...
// a ball that is not in motion (i.e. still). 
// It has a number (cue ball is 0) and a position on the table
typedef struct {
    unsigned char number;
    phylib_coord pos;
} phylib_still_ball;

// a ball that is rolling (i.e. not still). 
// It has a number (cue ball is 0) and a position on the table. 
// It also has a velocity and a (negative) acceleration (due to friction)
typedef struct {
    unsigned char number;
    phylib_coord pos;
    phylib_coord vel;
...
// a horizontal cushion (i.e. a cushion along either of the two the short sides of the table). 
// It has only a y-coordinate.
typedef struct {
    double y;
} phylib_hcushion;

// a vertical cushion (i.e. a cushion along either of the two the long sides of the table). 

...
class Table( phylib.phylib_table ):
    ...
    def eightBall(self):
        """
        Helper method to find the 8 ball (number 8) in the table.
        """
        for ball in self:
            if isinstance(ball, StillBall):
    ...
    def otherBalls(self):
        """
        Helper method to find all the balls on the table except the cue ball and the eight ball.
        Returns a list of ball objects.
        """
        other_balls = []
        for ball in self:
            if isinstance(ball, StillBall) and ball.obj.still_ball.number != 0 and ball.obj.still_ball.number != 8:
                other_balls.append(ball)
        return other_balls

    def balls1To7(self):
        """
        Helper method to find all balls with numbers 1-7 on the table.
        Returns a list of ball objects.
        """
        balls_1_to_7 = []
        for ball in self:
            if isinstance(ball, StillBall) and 1 <= ball.obj.still_ball.number <= 7:
                balls_1_to_7.append(ball)
        return balls_1_to_7

    def balls9To15(self):
        """
        Helper method to find all balls with numbers 9-15 on the table.
        Returns a list of ball objects.
        """
        balls_9_to_15 = []
        for ball in self:
            if isinstance(ball, StillBall) and 9 <= ball.obj.still_ball.number <= 15:
                balls_9_to_15.append(ball)
        return balls_9_to_15
    ...
    def initializeEntireTable(self):
        ...
        for i, row in enumerate(rows):
            for j in range(row):
                if i % 2 == 0:
                    x = TABLE_WIDTH / 2.0 - (row - 1) * (BALL_DIAMETER + 4.0) / 2 + j * (BALL_DIAMETER + 4.0) + nudge()
                else:
                    x = TABLE_WIDTH / 2.0 - j * (BALL_DIAMETER + 4.0) + (row - 1) * (BALL_DIAMETER + 4.0) / 2 + nudge()
                y = TABLE_WIDTH / 2.0 + i * math.sqrt(3.0) / 2.0 * (BALL_DIAMETER + 4.0) + nudge()
...
class Game:
    ...
    def shoot( self, gameName, playerName, table, xvel, yvel, low ):
        """
        ...
        other_balls = table.otherBalls()
        num_other_balls = len(other_balls)
        ...
        segment = table;
        svg_array = [];  # Array to store SVG representations
        ...
        other = table.otherBalls()
        
        eight_ball = table.eightBall();

        if (low == 1):
            half = table.balls1To7();
        else:
            half = table.balls9To15();

        print("TESTTT HALF:", half)

        numShot = num_other_balls - len(other) - len(half)

        print("HOWMANY;", numShot)

        if eight_ball is None and half is not None:
            print("U LSOEEE")
            return svg_array, table, playerName;
        elif eight_ball is None and half is None:
            print("WINNN")
            return svg_array, table, playerName, numShot;

        return svg_array, table, playerName, numShot, gameName;
